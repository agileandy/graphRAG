...
def hybrid_search(query: str, n_results: int = 5, max_hops: int = 2) -> None:
    ...
    try:
        ...
        for i in range(min(len(ids), n_results)):
            ...
            logger.info(
                f"  Relevance: {1 - distance if distance is not None else 'N/A'}"
            )
        ...
        for i, result in enumerate(graph_results[:n_results]):
            ...
            if related_concepts:
                ...
                for j, related in enumerate(related_concepts[:5]):
                    ...
                    logger.info(
                        f"    - {related_name} (Strength: {related_strength:.2f})"
                    )
