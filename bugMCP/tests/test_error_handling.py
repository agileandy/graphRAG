...
from mcp.shared.exceptions import (
    McpError,
)  # Import McpError for more specific exception checking
...
async def test_call_tool_with_missing_required_params(mcp_session: ClientSession):
    ...
    # For the current mcp_server.py, it might be a generic error
    ...
    # The current `bugMCP_mcp.py` server's `add_bug` tool would raise a TypeError
    # if params are missing
    # which the `FastMCP` layer would then convert into an McpError.
    # The `src/mcp/mcp_server.py`'s `handle_add_document` (if used for `add_bug`)
    # returns an error dict.
    assert exc_info.value.error.code != 0  # Not a success code
    assert (
        "missing" in exc_info.value.error.message.lower()
        or "required" in exc_info.value.error.message.lower()
    )
...
async def test_call_tool_with_invalid_param_type(mcp_session: ClientSession):
    ...
    # If the tool handler (e.g., handle_get_bug if it existed) tried to use
    # a non-int ID,
    ...
    assert (
        exc_info.value.error.code == -32602 or exc_info.value.error.code == -32603
...
async def test_delete_nonexistent_bug(mcp_session: ClientSession):
    ...
    )  # Or success with a message indicating nothing was deleted
