...
def visualize_concepts(
    neo4j_db: Neo4jDatabase, limit: int = 50, output_file: str = "concepts_graph.html"
) -> None:
    ...
    for rel in relationships:
        ...
        # Add edge
        net.add_edge(
            source_id, target_id, value=strength * 5, title=f"Strength: {strength}"
        )
...
def visualize_book_concepts(
    neo4j_db: Neo4jDatabase,
    book_title: str = None,
    output_file: str = "book_concepts_graph.html",
) -> None:
    ...
    for result in results:
        ...
        if book_id not in added_nodes:
            net.add_node(
                book_id, label=book_title, title=book_title, color="#ff9999", size=30
            )
...
def visualize_concept_neighborhood(
    ...
    concept_name: str,
    max_hops: int = 2,
    output_file: str = "concept_neighborhood.html",
) -> None:
    ...
    """
    print(
        f"Creating visualization of concept neighborhood: {concept_name} (max hops: {max_hops})..."
    )
    ...
    net.add_node(
        concept_id, label=concept_name, title=concept_name, color="#ff9999", size=30
    )
    ...
    for result in results:
        ...
        if target_id not in added_nodes:
            net.add_node(
                target_id, label=target_name, title=target_name, color=color, size=20
            )
            added_nodes.add(target_id)
...
def export_graph_data(
    neo4j_db: Neo4jDatabase, output_file: str = "graph_data.json"
) -> None:
...
def main() -> None:
    ...
    parser.add_argument(
        "--concepts",
        action="store_true",
        help="Visualize concepts and their relationships",
    )
    parser.add_argument(
        "--books", action="store_true", help="Visualize books and their concepts"
    )
    parser.add_argument(
        "--book", type=str, help="Visualize a specific book and its concepts"
    )
    parser.add_argument(
        "--concept", type=str, help="Visualize a concept and its neighborhood"
    )
    parser.add_argument(
        "--export", action="store_true", help="Export graph data to a JSON file"
    )
    parser.add_argument(
        "--limit", type=int, default=50, help="Maximum number of nodes to include"
    )
    parser.add_argument(
    ...
    if args.concepts:
        ...
        while True:
            ...
            if choice == "1":
                ...
                output_file = (
                    input("Output file (default: concepts_graph.html): ")
                    or "concepts_graph.html"
                )
                visualize_concepts(neo4j_db, limit, output_file)
            elif choice == "2":
                output_file = (
                    input("Output file (default: all_books_graph.html): ")
                    or "all_books_graph.html"
                )
                visualize_book_concepts(neo4j_db, None, output_file)
            elif choice == "3":
                book_title = input("Enter book title: ")
                output_file = (
                    input("Output file (default: book_concepts_graph.html): ")
                    or "book_concepts_graph.html"
                )
                ...
                output_file = (
                    input("Output file (default: concept_neighborhood.html): ")
                    or "concept_neighborhood.html"
                )
                visualize_concept_neighborhood(
                    neo4j_db, concept_name, hops, output_file
                )
            elif choice == "5":
                output_file = (
                    input("Output file (default: graph_data.json): ")
                    or "graph_data.json"
                )
                export_graph_data(neo4j_db, output_file)
            elif choice == "6":
