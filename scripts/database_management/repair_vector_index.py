...
def main() -> bool:
    ...
    parser.add_argument(
        "--force",
        action="store_true",
        help="Force repair even if index appears healthy",
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify index health after repair"
    )
    ...
    try:
        result = vector_db.query(
            query_texts=["test query after index repair"], n_results=1
        )
        ...
        if result and "documents" in result and len(result["documents"]) > 0:
            ...
            print(
                "⚠️ Vector search returned no results (this might be normal for an empty database)"
            )
