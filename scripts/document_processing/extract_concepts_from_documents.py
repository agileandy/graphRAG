...
def extract_concepts_from_documents(
    ...
    for doc in tqdm(documents, desc="Processing documents"):
        ...
        filtered_concepts = [
            c for c in weighted_concepts if c.get("weight", 0) >= min_concept_weight
        ]
        ...
        for concept in filtered_concepts:
            ...
            if concept_node:
                ...
                graph_db.add_document_concept_relationship(
                    doc_id, concept_node["id"], weight=concept_weight
                )
...
def main() -> None:
    ...
    parser = argparse.ArgumentParser(
        description="Extract concepts from documents in the vector database"
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        type=int,
        default=2,
        help="Minimum concept length in words",
    )
    parser.add_argument(
        "--max-concept-length",
        type=int,
        default=5,
        help="Maximum concept length in words",
    )
    parser.add_argument(
        "--min-concept-weight",
        type=float,
        default=0.5,
        help="Minimum concept weight to include",
    )
    parser.add_argument(
        "--use-nlp", action="store_true", help="Use NLP-based extraction"
    )
    parser.add_argument(
        "--use-llm", action="store_true", help="Use LLM-based extraction"
    )
    parser.add_argument(
        "--domain",
        type=str,
        default="tech",
        choices=["general", "tech", "academic"],
        help="Domain for stopwords",
    )
    ...
    print(
        f"Using NLP: {args.use_nlp}, Using LLM: {args.use_llm}, Domain: {args.domain}"
    )
    print(
        f"Min concept length: {args.min_concept_length}, Max concept length: {args.max_concept_length}"
    )
