    ...
    try:
        with driver.session() as session:
            ...
            for record in results:
                ...
                if "Concept" in description and "name" in description and "UNIQUE" in description:
                     present_constraints["concept_name_unique"] = description
                elif "Document" in description and "id" in description and "UNIQUE" in description:
            ...
            for key, desc_pattern in expected_constraints.items():
                ...
                if key in present_constraints:
                    print(f"  [PRESENT] Constraint for '{key}' found: {present_constraints[key]}")
                else:
                    # Fallback to check if any description contains the core elements
                    found_by_desc = False
                    for pc_desc in present_constraints.values():
                        if ("Concept" in pc_desc and "name" in pc_desc and "UNIQUE" in pc_desc and key == "concept_name_unique") or \
                           ("Document" in pc_desc and "id" in pc_desc and "UNIQUE" in pc_desc and key == "document_id_unique"):
                            print(f"  [PRESENT] Constraint for '{key}' found (matched by description): {pc_desc}")
                            found_by_desc = True
                            break
                    if not found_by_desc:
                         print(f"  [MISSING] Constraint for '{key}' (expected pattern: {desc_pattern})")
