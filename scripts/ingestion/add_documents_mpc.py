...
try:
    ...
    print("WARNING: PyPDF2 is not installed. Client-side PDF processing ('auto' mode) will not be available.")
    print("To enable client-side PDF processing, please install it with 'pip install PyPDF2'.")
...
def connect_to_mpc(url: str = DEFAULT_MPC_URL, verbose: bool = False):
    ...
    try:
        ...
        if ping_response.get("status") != "success":
            print(f"⚠️ Warning: MPC ping failed or returned non-success: {ping_response.get('message', 'No message')}")
            if verbose: print(f"Ping response details: {ping_response}")
        elif verbose:
...
def send_request(conn, action: str, verbose: bool = False, suppress_exit: bool = False, **kwargs) -> Dict[str, Any]:
    ...
    if verbose:
        ...
        for k, v in kwargs.items():
            if isinstance(v, str) and len(v) > 200: # Avoid printing very large text content
    ...
    try:
        ...
        if suppress_exit:
            return {"status": "error", "message": "Timeout waiting for MPC server response"}
        sys.exit(1)
    except Exception as e:
...
def extract_text_from_pdf(file_path: str, verbose: bool = False) -> str:
    ...
    if not PyPDF2:
        print("❌ PyPDF2 is not available. Cannot extract text from PDF for client-side processing.")
        return ""
...
def chunk_text(text: str, max_chunk_size: int, verbose: bool = False) -> List[str]:
    ...
    if verbose:
        print(f"Text length ({len(text)}) exceeds max_chunk_size ({max_chunk_size}). Splitting into chunks...")
...
def generate_metadata(file_path: str, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if author == "Unknown" and "(" in title and ")" in title:
        try:
            ...
            if potential_author and "Z-Library" not in potential_author and len(potential_author) < 70 and not potential_author.isdigit():
                author = potential_author
                title = title.split("(")[0].strip() # Clean title if author was extracted
        except IndexError:
            pass # Ignore if parsing fails

    category = args.category
    if not category and args.input_path and os.path.isdir(args.input_path): # Try to infer from parent directory if input is a dir
        ...
        if parent_dir_name and parent_dir_name != input_base and parent_dir_name != "." and len(parent_dir_name) > 1:
    ...
    metadata = {
        ...
        "original_file_path": os.path.abspath(file_path) # Store absolute path of original file
    }
    if args.subcategory:
        metadata["subcategory"] = args.subcategory
    if args.concepts:
        # Split concepts by comma, strip whitespace
        metadata["concepts"] = [c.strip() for c in args.concepts.split(",") if c.strip()]
    if args.custom_metadata:
        try:
            custom_data = json.loads(args.custom_metadata)
            metadata.update(custom_data)
        except json.JSONDecodeError:
            print(f"⚠️ Warning: Could not parse custom_metadata JSON: {args.custom_metadata}")

    return metadata

def process_single_file_client_side(conn, file_path: str, args: argparse.Namespace) -> Dict[str, Any]:
    ...
    if not text:
        return {"status": "error", "message": f"Failed to extract text from {file_path}"}
    ...
    for i, chunk_content in enumerate(text_chunks):
        ...
        if len(text_chunks) > 1:
            chunk_metadata["title"] = f"{doc_metadata['title']} - Part {i+1}/{len(text_chunks)}"
            chunk_metadata["chunk_index"] = i
            chunk_metadata["total_chunks"] = len(text_chunks)

        if args.verbose:
            print(f"Adding chunk {i+1}/{len(text_chunks)} for '{doc_metadata['title']}' ({len(chunk_content)} chars)")

        if args.dry_run:
            print(f"[DRY RUN] Would send 'add-document' for chunk {i+1} of {doc_metadata['title']}")
            if args.verbose: print(f"[DRY RUN] Metadata: {json.dumps(chunk_metadata, indent=1)}")
            results.append({"status": "dry_run_success", "message": "Dry run: document chunk not sent."})
            continue

        result = send_request(conn, "add-document", verbose=args.verbose, text=chunk_content, metadata=chunk_metadata)
        results.append(result)
        if result.get("status") != "success" and args.verbose:
            print(f"⚠️ Warning: Failed to add chunk {i+1}: {result.get('message', 'Unknown error')}")
        if i < len(text_chunks) -1 : time.sleep(0.1) # Small delay between chunks

    successful_chunks = sum(1 for r in results if r.get("status") == "success" or r.get("status") == "dry_run_success")
    if successful_chunks == len(text_chunks):
        return {"status": "success", "message": f"Document '{doc_metadata['title']}' (client-side) added successfully in {len(text_chunks)} chunk(s)."}
    else:
        return {"status": "error", "message": f"Document '{doc_metadata['title']}' (client-side) partially added. {successful_chunks}/{len(text_chunks)} chunks successful."}


def process_input_path(conn, args: argparse.Namespace):
    ...
    if os.path.isfile(input_path):
        ...
        if args.mode == "server-folder": # Special mode to send whole folder path to server
            if args.dry_run:
                print(f"[DRY RUN] Would send 'add-folder' for directory: {input_path}")
                doc_metadata = generate_metadata(input_path, args) # Use dir path for generic metadata
                if args.verbose: print(f"[DRY RUN] Metadata: {json.dumps(doc_metadata, indent=1)}")
                print("\n=== Processing Summary ===")
                ...
                doc_metadata = generate_metadata(input_path, args) # Use dir path for generic metadata
                result = send_request(conn, "add-folder", verbose=args.verbose, folder_path=os.path.abspath(input_path), metadata=doc_metadata)
                print("\n=== Processing Summary ===")
                print(f"Total folders found: 1")
                print(f"Folders attempted: 1")
                if result.get("status") == "success":
                    print(f"Successfully processed: 1. Server message: {result.get('message', 'OK')}")
                else:
                    print(f"Failed: 1. Server message: {result.get('message', 'Error')}")
                return # Exit after server-folder mode

        # For 'auto' and 'server-file' modes, find individual PDFs
        glob_pattern = os.path.join(input_path, "**", "*.pdf") if args.recursive else os.path.join(input_path, "*.pdf")
    ...
    for i, file_path in enumerate(files_to_process):
        ...
        if args.mode == "auto":
            if not PyPDF2:
                print(f"⚠️ Skipping {file_path}: PyPDF2 not available for client-side processing and mode is 'auto'. Consider '--mode server-file'.")
                error_count +=1
                continue
            ...
            doc_metadata = generate_metadata(file_path, args)
            if args.dry_run:
                print(f"[DRY RUN] Would send 'add-document' (server-file mode) for file: {file_path}")
                if args.verbose: print(f"[DRY RUN] Metadata: {json.dumps(doc_metadata, indent=1)}")
                result = {"status": "dry_run_success", "message": "Dry run: document not sent."}
            else:
                if args.verbose: print(f"Processing file (server-file mode): {file_path}")
                result = send_request(conn, "add-document", verbose=args.verbose, file_path=os.path.abspath(file_path), metadata=doc_metadata)
        ...
        processed_count +=1
        if result and (result.get("status") == "success" or result.get("status") == "dry_run_success"):
            success_count += 1
            print(f"✅ {result.get('message', 'Successfully processed.')}")
        else:
            error_count += 1
            print(f"❌ {result.get('message', 'Failed to process.')}")
            if args.verbose and result: print(f"Full error response: {result}")
...
"""
    ...
    parser.add_argument("--url", default=DEFAULT_MPC_URL, help=f"MPC server URL (default: {DEFAULT_MPC_URL})")

    # Input arguments
    parser.add_argument("--input-path", required=True, help="Path to a PDF file or a directory containing PDFs.")
    parser.add_argument("--recursive", action="store_true", help="If input-path is a directory, process subdirectories recursively.")
    ...
    # Metadata arguments
    parser.add_argument("--category", type=str, help="Category for the document(s). If not provided, inferred from parent directory or 'General'.")
    parser.add_argument("--subcategory", type=str, help="Subcategory for the document(s).")
    parser.add_argument("--author", type=str, help="Author for the document(s). If not provided, 'Unknown' or inferred from filename.")
    parser.add_argument("--source", type=str, help="Source of the document(s) (e.g., 'project_docs', 'research_papers'). Default 'local_import_script'.")
    parser.add_argument("--concepts", type=str, help="Comma-separated list of concepts related to the document(s).")
    parser.add_argument("--custom-metadata", type=str, help="JSON string for additional custom metadata fields (e.g., '{\"project_id\": \"X123\", \"status\": \"draft\"}')")

    # Client-side processing arguments (for 'auto' mode)
    parser.add_argument("--no-chunking", action="store_true", help="Disable client-side text chunking (sends entire document text as one piece).")
    ...
    # General arguments
    parser.add_argument("--delay", type=float, default=0.5, help="Delay in seconds between processing files (default: 0.5s).")
    parser.add_argument("--verbose", "-v", action="store_true", help="Enable verbose output, including request/response details.")
    parser.add_argument("--dry-run", action="store_true", help="Simulate processing without actually sending data to MPC or writing files.")

    args = parser.parse_args()

    if args.mode == "auto" and not PyPDF2:
        print("❌ Error: Mode 'auto' (client-side processing) selected, but PyPDF2 is not installed.")
        print("Please install PyPDF2 ('pip install PyPDF2') or choose '--mode server-file' or '--mode server-folder'.")
        sys.exit(1)
    ...
    conn = None
    try:
        if not args.dry_run: # No need to connect for dry run if all processing is client-side preview
        ...
        if conn:
            if args.verbose: print("Closing MPC connection...")
            conn.close()
            if args.verbose: print("MPC connection closed.")
