...
def optimize_vector_database(
    ...
    for i, doc_id in enumerate(all_data.get("ids", [])):
        ...
        source_id = metadata.get("doc_id", "").split("-chunk-")[0] if "-chunk-" in metadata.get("doc_id", "") else metadata.get("doc_id", "")
    ...
    for source_id, group in doc_groups.items():
        ...
        if not dry_run:
            ...
            print(f"  Replaced {len(group['documents'])} chunks with {len(chunks)} optimized chunks")
            optimized_count += 1
        else:
            print(f"  Would replace {len(group['documents'])} chunks with optimized chunks (dry run)")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Optimize existing data in the GraphRAG system")
    ...
    if not args.yes and not args.dry_run:
        response = input("This will optimize all existing data in the vector database. Continue? (y/n): ")
        if response.lower() != "y":
