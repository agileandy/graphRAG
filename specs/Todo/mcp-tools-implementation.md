...
This document provides detailed implementation guidance for creating the MCP tools for the GraphRAG project.

## Overview

The MCP server needs to implement 11 tools that allow AI agents to interact with the GraphRAG system. These tools will be implemented as handlers in the MCP server and will bridge between the MCP protocol and the existing GraphRAG functionality.

## Tool Definitions

Each tool needs:
1. A JSON Schema definition for its parameters
2. A handler function that processes the parameters and returns results
3. Error handling for invalid parameters or failed operations

## Implementation Details

### 1. Tool Definitions

Add these tool definitions to the `TOOLS` list in `src/mpc/mcp_server.py`:
...
Add these tool handlers to `src/mpc/mcp_server.py`:
    ...
    try:
        ...
        if process_async:
            ...
            job_manager.run_job_async(
                job=job,
                task_func=add_document_to_graphrag,
                task_args=(text, metadata, neo4j_db, vector_db, duplicate_detector)
            )
...
### 3. Update Tool Handlers Map

Update the `TOOL_HANDLERS` map in `src/mpc/mcp_server.py`:
...
After implementing the MCP tools, the next steps will be:

1. Create client configuration for AI agent integration
2. Enhance tests to verify all tools work correctly
