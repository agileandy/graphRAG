...
def test_small_text_extraction():
    ...
    if concepts:
        ...
        for concept in concepts:
            print(f"  - {concept.get('concept', 'Unknown')} (relevance: {concept.get('relevance', 'Unknown')})")
    else:
...
def test_very_large_text_extraction():
    ...
    # Extract concepts
    print("Extracting concepts from very large text (should trigger optimized chunking)...")
    concepts = extractor.extract_concepts(test_text)
...
def main():
    ...
    print(f"Very Large Text Extraction: {'✅ Passed' if very_large_ok else '❌ Failed'}")
