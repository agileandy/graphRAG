...
try:
    ...
    print(f"Error importing Flask app: {e}. Ensure PYTHONPATH is correct and app dependencies are mockable.")
    app = None # Fallback
...
def client():
    ...
    if app is None:
        pytest.fail("Flask app could not be imported. Check test setup and src.api.server dependencies.")
...
def test_add_document_duplicate(MockDuplicateDetector, mock_add_document_to_graphrag, client): # Order of mocks is important
    ...
    mock_detector_instance.is_duplicate.return_value = (True, "existing_doc_456", "hash_match")
...
def test_add_document_missing_text(client):
    ...
    response = client.post('/documents', json={
        "metadata": {"source": "test_missing_text"}
    })
...
@patch('scripts.document_processing.add_document_core.add_document_to_graphrag')
def test_add_document_unexpected_result_from_core(mock_add_document_to_graphrag, client):
...
@patch('scripts.document_processing.add_document_core.add_document_to_graphrag', side_effect=Exception("Unhandled core error"))
def test_add_document_unhandled_exception_in_core(mock_add_document_to_graphrag, client):
    ...
    assert "Unhandled exception: Unhandled core error" in data["error"] # Check specific error message
    assert data["document_id"] is None # Key check for the bug fix
...
def test_add_document_unhandled_exception_early(mock_request_json_prop, client):
    ...
    mock_request_json_prop.side_effect = RuntimeError("Simulated failure accessing request.json")

    # The actual data sent doesn't matter as much since request.json is mocked to fail.
    response = client.post('/documents', data='{"text":"test"}', content_type='application/json')

    assert response.status_code == 500
    data = json.loads(response.data)
    assert "error" in data
    assert "Unhandled exception: Simulated failure accessing request.json" in data["error"]
    assert data["document_id"] is None # Key check for the bug fix
    assert "traceback" in data
