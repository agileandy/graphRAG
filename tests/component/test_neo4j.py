...
def test_graph_database(
    ...
    try:
        document = graph_db.add_document(
            "doc-test",
            {
                "title": "Test Document",
                "author": "Test Author",
    ...
    try:
        relationship = graph_db.add_document_concept_relationship(
            document["id"], concept["id"], "MENTIONS", {"weight": 0.8}
        )
    ...
    try:
        ...
        logger.info(
            f"Counts: {concept_count} concepts, {document_count} documents, {relationship_count} relationships"
        )
    except Exception as e:
