...
def test_deduplication_with_db() -> bool:
    ...
    vector_db.add_documents(
        documents=[doc1_text], metadatas=[doc1_metadata], ids=[doc1_id]
    )
    print(f"✅ Added first document with ID: {doc1_id}")

    # Check if second document is detected as duplicate
    is_dup, dup_id, method = is_likely_duplicate(
        doc2_text, doc2_metadata, vector_db.collection
    )
    ...
    is_dup, dup_id, method = is_likely_duplicate(
        doc3_text, doc3_metadata, vector_db.collection
    )
    ...
    if not is_dup:
        ...
        print(
            f"❌ Non-duplicate incorrectly identified as duplicate (method: {method})"
        )
