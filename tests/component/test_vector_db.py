...
def main():
    ...
    parser.add_argument("--query", type=str, default="neural networks", help="Search query")
    parser.add_argument("--n-results", type=int, default=5, help="Number of results to return")
    parser.add_argument("--repair", action="store_true", help="Attempt to repair the index if needed")
    ...
    try:
        ...
        if vector_results and "documents" in vector_results and vector_results["documents"]:
            print(f"✅ Vector search successful! Found {len(vector_results['documents'][0])} results.")
            ...
            if args.verbose:
                ...
                for i, doc in enumerate(vector_results["documents"][0]):
                    ...
                    if "metadatas" in vector_results and i < len(vector_results["metadatas"][0]):
    ...
    try:
        hybrid_results = db_linkage.hybrid_search(
            ...
            n_vector_results=args.n_results,
            max_graph_hops=2
        )
        
        # Display results
        if hybrid_results:
            vector_count = len(hybrid_results.get("vector_results", {}).get("documents", []))
            graph_count = len(hybrid_results.get("graph_results", []))
            
            print(f"✅ Hybrid search successful! Found {vector_count} vector results and {graph_count} graph results.")
            
            if args.verbose:
                print("\nVector Results:")
                for i, doc in enumerate(hybrid_results.get("vector_results", {}).get("documents", [])):
                    print(f"\n{i+1}. {doc[:200]}...")
