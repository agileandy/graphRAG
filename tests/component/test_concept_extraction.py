...
def test_llm_based_extraction():
    ...
    if not OPENAI_AVAILABLE:
        print("⚠️ OpenAI not available or API key not set. Skipping LLM-based extraction test.")
        return False
    ...
    for domain, text in SAMPLE_TEXTS.items():
        ...
        if weighted_concepts:
            ...
            for concept in weighted_concepts:
                print(f"  - {concept['concept']} (weight: {concept.get('weight', 0):.2f}, frequency: {concept.get('frequency', 0)})")
        else:
...
def main():
    ...
    print(f"Domain-specific Extraction: {'✅ Passed' if domain_specific_ok else '❌ Failed'}")
