...
try:
    from src.processing.concept_extractor import ConceptExtractor, smart_chunk_text, _is_valid_concept, DOMAIN_STOPWORDS, load_llm_config as load_concept_extractor_llm_config
    from src.llm.llm_provider import create_llm_provider, OllamaProvider, load_llm_config as load_provider_llm_config
except ImportError:
    pytest.fail("Could not import processing/LLM classes. Make sure the files exist and paths are correct.")
...
def test_smart_chunk_text_basic():
    ...
    chunks = smart_chunk_text(text, chunk_size=50, overlap=10) # smart_chunk_text in concept_extractor might have different signature
    assert len(chunks) > 0
    ...
    assert all(len(chunk) <= 50 + 10 for chunk in chunks) # Max size can be slightly larger due to word boundaries
...
def test_smart_chunk_text_with_paragraphs():
    ...
    chunks = smart_chunk_text(text, chunk_size=100, overlap=20, semantic_boundaries=True)
    assert len(chunks) >= 2
...
@patch('src.processing.concept_extractor.os.getenv') # Patch getenv used by load_llm_config in concept_extractor
...
def test_load_concept_extractor_llm_config_openrouter_env_priority(mock_open_file, mock_json_load, mock_getenv):
    """Test loading OpenRouter key from env var priority for concept_extractor (Bug 35)."""
    mock_getenv.side_effect = lambda key, default=None: "env_openrouter_key" if key == "OPENROUTER_API_KEY" else default
    ...
    mock_open_file.return_value.__enter__.return_value = MagicMock() # Mock file context
...
def test_load_concept_extractor_llm_config_openrouter_placeholder(mock_logger, mock_open_file, mock_json_load, mock_getenv):
...
def test_ollama_provider_embedding_endpoint(mock_post):
    ...
    provider = OllamaProvider(api_base="http://localhost:11434", model="llama2", embedding_model="nomic-embed-text")
...
@patch('src.processing.concept_extractor.smart_chunk_text') # Mock the one concept_extractor._chunk_text calls
def test__concept_extractor_chunk_text_robustness(mock_smart_chunk_text_dep):
    ...
    assert isinstance(chunks, list), "Should return a list even if dependency returns None"
    assert all(isinstance(c, str) for c in chunks), "All items in list should be strings"
    ...
    chunks = extractor._chunk_text("some text", 100)
    assert isinstance(chunks, list), "Should return a list even if dependency returns a string"
    assert all(isinstance(c, str) for c in chunks), "All items in list should be strings"
    ...
    chunks = extractor._chunk_text("some text", 100)
    assert isinstance(chunks, list), "Should return a list even if dependency raises error"
    assert all(isinstance(c, str) for c in chunks), "All items in list should be strings"
...
def test_create_llm_provider_type_casting(mock_openrouter, mock_openai, mock_ollama):
    ...
    mock_ollama.assert_called_once_with(
        ...
        temperature=0.0, max_tokens=1000, timeout=60 # Defaults
    )
