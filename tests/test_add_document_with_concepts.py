...
from scripts.document_processing.add_document_with_concepts import add_document_to_graphrag
...
class TestAddDocumentWithConcepts(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.assertTrue(self.vector_db.verify_connection(), "Vector database connection failed!")
    ...
    def test_add_document_with_concepts_success(self):
        """
        ...
        # Assertions
        self.assertIsNotNone(result.get("document_id"), "Document ID should not be None")
        self.assertGreater(len(result.get("entities", [])), 0, "Should extract at least one entity")
        self.assertGreater(len(result.get("relationships", [])), 0, "Should extract at least one relationship")
