import pytest
import time
from tests.regression.test_utils import start_services, stop_services, add_test_document, search_documents, get_concept, get_documents_for_concept
...
def test_chroma_search():
    ...
    try:
        ...
        # Add a test document
        add_success, add_response = add_test_document(CHROMA_TEST_DOC_TEXT, CHROMA_TEST_DOC_METADATA)
        assert add_success, f"Failed to add Chroma test document: {add_response.get('error', 'Unknown error')}"
        ...
        assert search_success, f"Chroma search failed: {search_response.get('error', 'Unknown error')}"
        ...
        assert len(documents) > 0 and len(documents[0]) > 0, "Chroma search returned no documents"
        assert CHROMA_TEST_DOC_METADATA["title"] in metadatas[0][0].get('title', ''), "Chroma search did not return the expected document"
...
def test_hybrid_search():
    ...
    try:
        ...
        add_success_ai, add_response_ai = add_test_document(HYBRID_TEST_DOC_AI_TEXT, HYBRID_TEST_DOC_AI_METADATA)
        assert add_success_ai, f"Failed to add AI test document: {add_response_ai.get('error', 'Unknown error')}"
        print("AI test document added successfully.")

        add_success_ml, add_response_ml = add_test_document(HYBRID_TEST_DOC_ML_TEXT, HYBRID_TEST_DOC_ML_METADATA)
        assert add_success_ml, f"Failed to add ML test document: {add_response_ml.get('error', 'Unknown error')}"
        ...
        search_success, search_response = search_documents(search_query, n_results=1, max_hops=1)
        assert search_success, f"Hybrid search failed: {search_response.get('error', 'Unknown error')}"
        ...
        assert len(vector_results.get('documents', [])) > 0, "Hybrid search returned no vector results"
        assert len(graph_results.get('nodes', [])) > 0 or len(graph_results.get('relationships', [])) > 0, "Hybrid search returned no graph results"
        ...
        assert any(HYBRID_TEST_DOC_ML_METADATA["title"] in meta.get('title', '') for meta_list in vector_metadatas for meta in meta_list), "ML document not found in vector results"

        graph_nodes = graph_results.get('nodes', [])
        assert any(node.get('properties', {}).get('name') == 'Artificial Intelligence' for node in graph_nodes), "Artificial Intelligence concept not found in graph results"
...
def test_concept_relationships():
    ...
    try:
        ...
        add_success_rel, add_response_rel = add_test_document(CONCEPT_RELATIONSHIP_DOC_TEXT, CONCEPT_RELATIONSHIP_DOC_METADATA)
        assert add_success_rel, f"Failed to add concept relationship document: {add_response_rel.get('error', 'Unknown error')}"
        ...
        add_success_book, add_response_book = add_test_document(book_doc_text, book_doc_metadata)
        assert add_success_book, f"Failed to add book document: {add_response_book.get('error', 'Unknown error')}"
        ...
        assert concept_success, f"Failed to get concept '{concept_name}': {concept_response.get('error', 'Unknown error')}"
        ...
        docs_for_concept_success, docs_for_concept_response = get_documents_for_concept(concept_name)
        assert docs_for_concept_success, f"Failed to get documents for concept '{concept_name}': {docs_for_concept_response.get('error', 'Unknown error')}"
        ...
        assert any(doc.get('metadata', {}).get('title') == book_doc_metadata["title"] for doc in concept_docs), "Added book document not found when searching by concept"
        ...
        if process:
            stop_success = stop_services(process)
            assert stop_success, "Failed to stop services after concept relationships test"
