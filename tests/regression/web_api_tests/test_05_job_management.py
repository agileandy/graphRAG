...
import time
import uuid
from tests.regression.test_utils import start_services, stop_services, add_test_document, get_job_status, cancel_job
    ...
    try:
        ...
        for i in range(num_documents):
            ...
            if i == 0: # Capture job ID from the first document addition response
                ...
                assert job_id is not None, "Job ID not returned in add document response"
        ...
        while time.time() - start_time < timeout:
            ...
            assert status_success, f"Failed to get job status for {job_id}: {status_response.get('error', 'Unknown error')}"
        ...
        assert job_status == 'completed', f"Job {job_id} did not complete. Final status: {job_status}"
        ...
        if process:
            ...
            assert stop_success, "Failed to stop services after job status monitoring test"
...
def test_job_cancellation():
    ...
    try:
        ...
        for i in range(num_documents):
            ...
            if i == 0: # Capture job ID from the first document addition response
                job_id = add_response.get('job_id')
                assert job_id is not None, "Job ID not returned in add document response for cancellation test"
                print(f"Captured job ID for cancellation: {job_id}")

        assert job_id is not None, "No job ID was captured after adding documents for cancellation"
        print(f"All {num_documents} documents added. Attempting to cancel job {job_id}...")
        ...
        assert cancel_success, f"Failed to cancel job {job_id}: {cancel_response.get('error', 'Unknown error')}"
        ...
        while time.time() - start_time < timeout:
            status_success, status_response = get_job_status(job_id)
            assert status_success, f"Failed to get job status for {job_id} after cancellation: {status_response.get('error', 'Unknown error')}"
            job_status = status_response.get('status')
        ...
        assert job_status == 'cancelled', f"Job {job_id} did not get cancelled. Final status: {job_status}"
