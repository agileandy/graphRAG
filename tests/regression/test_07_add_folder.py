...
def add_folder(folder_path: str, file_types: Optional[List[str]] = None) -> Tuple[bool, Dict[str, Any]]:
    """
                    ...
                    if status_response.status_code == 200:
                        ...
                        if job_status in ['completed', 'failed', 'cancelled']:
                            ...
                            if job_status == 'completed':
                                ...
                                return False, {"error": f"Job {job_status}", "details": status_data}

                    # Wait before polling again
                    time.sleep(2)

                # If we get here, the job is still running after max_polls
                return True, {"status": "running", "job_id": job_id, "message": "Job still running, continuing with test"}

            # For synchronous processing or immediate response
            return True, response_data
        else:
            # Request failed
            error_message = f"Failed to add folder: {response.status_code} - {response.text}"
            print(error_message)
    ...
    try:
        ...
        if success:
            ...
            if graph_results:
                ...
                print("⚠️ No graph results found - this might be expected for new documents")
        else:
            print("❌ Search failed")
        ...
        if success:
            ...
            expected_concepts = [
                "Prompt", "Prompt Engineering", "LLM", "GPT", "AI", "Language Model"
            ]
            ...
            if found_count > 0:
                ...
                print("⚠️ None of the expected concepts were found - this might be due to different content")
        else:
