...
def test_deduplication() -> bool | None:
    ...
    try:
        ...
        if success:
            ...
            print(
                "⚠️ Document submission rejected - this might be expected if deduplication is working"
            )
            print(f"Error: {response.get('error')}")
        ...
        if success:
            ...
            if len(unique_names) == len(concept_names):
                ...
                # Find duplicates
                duplicates = [
                    name for name in unique_names if concept_names.count(name) > 1
                ]
