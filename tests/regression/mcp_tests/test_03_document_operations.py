import pytest
import uuid
from tests.regression.test_utils import start_services, stop_services, add_test_document, check_document_exists, get_document_count
...
def test_mcp_single_document():
    ...
    try:
        ...
        assert add_success, f"Failed to add single document via MCP: {add_response.get('error', 'Unknown error')}"
        ...
        count_success, count_response = get_document_count(use_mcp=True) # Assuming get_document_count supports MCP
        assert count_success, f"Failed to get document count via MCP: {count_response.get('error', 'Unknown error')}"
        assert count_response.get('count') == 1, f"Expected 1 document, found {count_response.get('count')}"
        ...
        if process:
            ...
            assert stop_success, "Failed to stop services after MCP single document test"
            print("Services stopped successfully after MCP single document test.")

def test_mcp_bulk_documents():
    """Add 50 docs via MCP"""
    ...
    try:
        ...
        count_success, count_response = get_document_count(use_mcp=True) # Assuming get_document_count supports MCP
        assert count_success, f"Failed to get document count via MCP: {count_response.get('error', 'Unknown error')}"
        assert count_response.get('count') == num_documents, f"Expected {num_documents} documents, found {count_response.get('count')}"
        print(f"Document count verified: {count_response.get('count')}")
...
def test_mcp_duplicate():
    ...
    try:
        ...
        # Add a single document
        doc_data = generate_unique_document(1)
        add_success_first, add_response_first = add_test_document(doc_data["text"], doc_data["metadata"], use_mcp=True) # Assuming use_mcp flag or similar
        assert add_success_first, f"Failed to add initial document via MCP: {add_response_first.get('error', 'Unknown error')}"
        print("Initial document added via MCP.")

        # Attempt to add the same document again
        add_success_second, add_response_second = add_test_document(doc_data["text"], doc_data["metadata"], use_mcp=True) # Assuming use_mcp flag or similar
        ...
        assert not add_success_second, f"Duplicate document addition via MCP unexpectedly succeeded: {add_response_second}"
        # Further checks could involve inspecting the error message or response structure
        print("Duplicate document addition via MCP handled as expected (failed or rejected).")

    finally:
        # Stop services
        if process:
            stop_success = stop_services(process)
            assert stop_success, "Failed to stop services after MCP duplicate document test"
