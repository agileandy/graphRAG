...
def test_database_deletion() -> None:
    ...
    try:
        ...
        assert mcp_connected_after_init, (
            "MCP server is not reachable after database initialization"
        )
        ...
        assert mcp_connected_after_clean, (
            "MCP server is not reachable after database cleaning"
        )
        ...
        if process:
            ...
            assert stop_success, (
                "Failed to stop services after MCP database deletion test"
            )
...
def test_database_initialization() -> None:
    ...
    try:
        ...
        mcp_connected_after_clean = test_mcp_connection()
        assert mcp_connected_after_clean, (
            "MCP server is not reachable after cleaning before initialization"
        )
        ...
        if process:
            stop_success = stop_services(process)
            assert stop_success, (
                "Failed to stop services after MCP database initialization test"
            )
            print(
                "Services stopped successfully after MCP database initialization test."
            )
