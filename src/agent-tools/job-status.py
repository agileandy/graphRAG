...
def display_job_status(result: dict[str, Any]) -> None:
    ...
    logger.info(
        f"Job {job_id} ({job_type}) status: {status}, progress: {progress:.1f}%"
    )
    logger.debug(f"Job {job_id} processed items: {processed}/{total}")
    logger.debug(
        f"Job {job_id} timestamps: created={created_at}, started={started_at}, completed={completed_at}"
    )
...
def main() -> int | None:
    ...
    parser = argparse.ArgumentParser(
        description="Check the status of a job in the GraphRAG system"
    )
