...
def main() -> int | None:
    ...
    parser = argparse.ArgumentParser(
        description="Retrieve passages about a specific concept"
    )
    parser.add_argument("--name", required=True, help="Name of the concept")
    parser.add_argument(
        "--limit", type=int, default=5, help="Maximum number of passages to return"
    )
    parser.add_argument(
    ...
    try:
        ...
        response = send_request(
            conn, "passages-about-concept", concept_name=args.name, limit=args.limit
        )
