...
import sys
import argparse
from utils import connect_to_mcp, send_request, get_mcp_url

def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Test if the GraphRAG MCP server is alive")
    ...
    try:
        ...
        if response.get("status") == "success":
            ...
            print(f"❌ MCP server returned an error: {response.get('error', 'Unknown error')}")
            return 1
    finally:
