...
def connect_to_mcp(url: Optional[str] = None) -> ws.ClientConnection:
    """
...
def send_request(conn: ws.ClientConnection, action: str, **kwargs) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(f"Sending message: {message_json[:200]}..." if len(message_json) > 200 else message_json)
        ...
        response = conn.recv()
        logger.debug(f"Received raw response: {response[:200]}..." if len(response) > 200 else response)
        ...
        if "jsonrpc" in response_data and response_data.get("jsonrpc") == "2.0":
            ...
            if "error" in response_data:
                ...
                logger.warning("JSON-RPC 2.0 response missing both 'error' and 'result' fields")
