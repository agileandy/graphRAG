...
class ConceptExtractor:
    ...
    def extract_concepts_llm(self, text: str, max_concepts: int = 10) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        if llm_manager is not None:
            try:
                ...
                logger.warning(f"Error using LLMManager for two-pass concept extraction: {e}")
                return {"concepts": [], "relationships": []}
        else:
            logger.warning("LLM-based extraction not available. Cannot perform two-pass extraction.")
            return {"concepts": [], "relationships": []}

    def _llm_pass1_extract_concepts_from_chunk(self, text_chunk: str) -> List[Dict[str, Any]]:
        """
        Pass 1: Extract concepts from a single text chunk using LLM.
        """
        max_text_length = 3000 # Max length for a chunk to send to LLM
        truncated_text = text_chunk[:max_text_length] + "..." if len(text_chunk) > max_text_length else text_chunk
        ...
        if llm_manager is None:
            ...
            return []
        try:
            response = llm_manager.generate(prompt, system_prompt=system_prompt, max_tokens=2000)
            if response.startswith("Error:") or response.startswith("API Response:"):
                logger.warning(f"LLM error during Pass 1 concept extraction: {response}")
                return []
    ...
    def _llm_pass2_analyze_relationships(self, concepts: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
        """
        Pass 2: Analyze relationships between a list of unique concepts using LLM.
        """
        concept_names = [concept["name"] for concept in concepts if isinstance(concept, dict) and "name" in concept and isinstance(concept["name"], str)]
        if not concept_names:
            logger.info("Pass 2: No valid concept names provided for relationship analysis.")
            return []

        max_concepts_for_analysis = 25
        if len(concept_names) > max_concepts_for_analysis:
            logger.info(f"Limiting relationship analysis to first {max_concepts_for_analysis} of {len(concept_names)} unique concepts.")
        ...
        if llm_manager is None:
            ...
            return []
        try:
            response = llm_manager.generate(prompt, system_prompt=system_prompt, max_tokens=3000)
            if response.startswith("Error:") or response.startswith("API Response:"):
                logger.warning(f"LLM error during Pass 2 relationship analysis: {response}")
                return []
            return self._parse_llm_json_response(response)
        except Exception as e:
        ...
        try:
            ...
            if json_array_start != -1 and json_array_end > json_array_start:
                ...
                if isinstance(parsed_data, list):
                    return [item for item in parsed_data if isinstance(item, dict)] # Filter for dicts
                else:
                    logger.warning(f"Parsed JSON from array markers is not a list. Got: {type(parsed_data)}. Response: {response[:200]}...")
            ...
            if json_obj_start != -1 and json_obj_end > json_obj_start:
                ...
                if isinstance(parsed_data, dict):
                    ...
                    logger.warning(f"Parsed JSON from object markers is not a dict or list. Got: {type(parsed_data)}. Response: {response[:200]}...")

            logger.debug(f"Could not find valid JSON array or object in LLM response: {response[:200]}...")
            return []
    ...
    def _extract_concepts_and_relationships_with_llm_manager(self, text: str) -> Dict[str, List[Dict[str, Any]]]:
        """
        Orchestrates the two-pass LLM concept and relationship extraction.
        """
        if llm_manager is None:
            logger.error("LLM manager is not initialized. Cannot perform two-pass extraction.")
            return {"concepts": [], "relationships": []}

        chunks = self._chunk_text(text, chunk_size=2500, overlap=300)
        logger.info(f"Split document into {len(chunks)} chunks for two-pass LLM extraction.")

        all_chunk_concepts: List[Dict[str, Any]] = []
        for i, chunk_text_item in enumerate(chunks):
            logger.info(f"Processing chunk {i+1}/{len(chunks)} for Pass 1 concept extraction ({len(chunk_text_item)} chars).")
            chunk_concepts = self._llm_pass1_extract_concepts_from_chunk(chunk_text_item)
            for concept in chunk_concepts:
                if isinstance(concept, dict):
                    concept["source_chunk_index"] = i
                    all_chunk_concepts.append(concept)
                else:
                    logger.warning(f"Skipping non-dict item from Pass 1 chunk {i+1}: {concept}")

        logger.info(f"Pass 1: Extracted {len(all_chunk_concepts)} raw concepts in total from all chunks.")

        unique_concepts_map: Dict[str, Dict[str, Any]] = {}
        for concept_item in all_chunk_concepts:
            if not isinstance(concept_item, dict) or "name" not in concept_item or not isinstance(concept_item["name"], str):
                logger.warning(f"Skipping invalid concept data during deduplication: {concept_item}")
                continue
            ...
            if name_lower not in unique_concepts_map:
                ...
                if new_desc and new_desc not in existing_concept.get("description", ""):
                    existing_concept["description"] = (existing_concept.get("description", "") + "; " + new_desc).strip("; ")

                new_related = concept_item.get("related_concepts", [])
                if isinstance(new_related, list):
                    existing_related_set = set(existing_concept.get("related_concepts", []))
                    existing_related_set.update(r_concept for r_concept in new_related if isinstance(r_concept, str))
                ...
                if "source_chunk_indices" not in existing_concept:
                    ...
                    existing_concept["source_chunk_indices"] = [first_occurrence_idx] if first_occurrence_idx is not None else []

                if current_chunk_idx is not None and current_chunk_idx not in existing_concept["source_chunk_indices"]:
        ...
        logger.info(f"Pass 1: Consolidated to {len(deduplicated_concepts_list)} unique concepts.")

        relationships: List[Dict[str, Any]] = []
        if deduplicated_concepts_list:
            logger.info(f"Starting Pass 2: Analyzing relationships among {len(deduplicated_concepts_list)} unique concepts.")
            relationships = self._llm_pass2_analyze_relationships(deduplicated_concepts_list)
            logger.info(f"Pass 2: Identified {len(relationships)} relationships.")
        else:
            logger.info("Skipping Pass 2 as no unique concepts were extracted in Pass 1.")
    ...
    def extract_concepts(self, text: str, method: str = "auto", max_concepts: int = 20) -> Dict[str, List[Dict[str, Any]]]:
        """
        ...
        """
        logger.info(f"ConceptExtractor: Extracting concepts from text of length {len(text)} using method: {method}, max_concepts: {max_concepts}")
        ...
        if method == "auto":
            if self.use_llm and llm_manager:
                ...
                logger.info("Auto method: LLM not available, selecting NLP-based extraction.")
                chosen_method = "nlp"
            else:
                logger.info("Auto method: LLM and NLP not available, selecting rule-based extraction.")
                chosen_method = "rule"

        if chosen_method == "llm":
            if self.use_llm and llm_manager:
                llm_output_data = self.extract_concepts_llm(text, max_concepts=max_concepts)
                output["concepts"] = llm_output_data.get("concepts", [])
                output["relationships"] = llm_output_data.get("relationships", [])
            else:
                logger.warning("LLM method selected, but LLM not available. Falling back.")
                if self.use_nlp:
                    chosen_method = "nlp"
                    logger.info("Falling back to NLP from LLM.")
                    concepts_list_nlp = self.extract_concepts_nlp(text)
                    output["concepts"] = [{"name": c, "type": "Unknown", "description": "Extracted via NLP (LLM fallback)", "source": "nlp_fallback_llm"} for c in concepts_list_nlp[:max_concepts]]
                else:
                    chosen_method = "rule"
                    logger.info("Falling back to rule-based from LLM (NLP unavailable).")
                    concepts_list_rule = self.extract_concepts_rule_based(text)
                    output["concepts"] = [{"name": c, "type": "Unknown", "description": "Extracted via rule-based (LLM fallback)", "source": "rule_fallback_llm"} for c in concepts_list_rule[:max_concepts]]
                output["relationships"] = []

        elif chosen_method == "nlp":
            if self.use_nlp:
                concepts_list_nlp = self.extract_concepts_nlp(text)
                output["concepts"] = [{"name": c, "type": "Unknown", "description": "Extracted via NLP", "source": "nlp"} for c in concepts_list_nlp[:max_concepts]]
            else:
                logger.warning("NLP method selected, but NLP not available. Falling back to rule-based.")
                chosen_method = "rule"
                concepts_list_rule = self.extract_concepts_rule_based(text)
                output["concepts"] = [{"name": c, "type": "Unknown", "description": "Extracted via rule-based (NLP fallback)", "source": "rule_fallback_nlp"} for c in concepts_list_rule[:max_concepts]]
            output["relationships"] = []

        elif chosen_method == "rule":
            concepts_list_rule = self.extract_concepts_rule_based(text)
            output["concepts"] = [{"name": c, "type": "Unknown", "description": "Extracted via rule-based", "source": "rule"} for c in concepts_list_rule[:max_concepts]]
            output["relationships"] = []

        else:
            logger.error(f"Unknown extraction method resolved: {chosen_method}. Defaulting to rule-based.")
            concepts_list_rule = self.extract_concepts_rule_based(text)
            output["concepts"] = [{"name": c, "type": "Unknown", "description": "Extracted via rule-based (unknown method fallback)", "source": "rule_fallback_unknown"} for c in concepts_list_rule[:max_concepts]]
            output["relationships"] = []
        ...
        logger.info(f"Finalizing extraction: {len(output['concepts'])} concepts and {len(output['relationships'])} relationships using effective method: {chosen_method}.")
        return output
    ...
    def _is_valid_concept(self, concept: str) -> bool:
        ...
        non_alnum_count = sum(1 for char in concept if not char.isalnum() and char not in allowed_internal_chars)
        if non_alnum_count > len(words) -1 : # Heuristic: more non-alnum than spaces between words
            return False
    ...
    def weight_concepts(self, concepts: List[Dict[str, Any]], text: str) -> List[Dict[str, Any]]:
        ...
        for concept_data in concepts:
            if isinstance(concept_data, dict) and "name" in concept_data:
                ...
                valid_concepts.append(concept_data)
                 ...
                 valid_concepts.append({"name": concept_data, "relevance": 0.5, "source": "unknown_str_to_dict"})
        ...
        for concept_data in valid_concepts:
            ...
            try:
                ...
                if frequency > 1:
                    concept_data["relevance"] = min(1.0, concept_data.get("relevance", 0.5) + (frequency * 0.05))

                # Positional bonus (e.g., if in the first 20% of text)
                if text_lower.find(concept_name_lower) < len(text_lower) * 0.2:
                     concept_data["relevance"] = min(1.0, concept_data.get("relevance", 0.5) + 0.1)

            except Exception as e:
                logger.warning(f"Error weighting concept '{concept_data.get('name', 'N/A')}': {e}")
