...
class Job:
    ...
    def update_progress(self, processed_items: int, total_items: int) -> None:
        ...
        logger.debug(f"Job {self.job_id} progress updated: {old_progress:.1f}% -> {self.progress:.1f}% ({processed_items}/{total_items})")
...
class JobManager:
                ...
                try:
                    ...
                    if job_id and job_type:
                        ...
                        if loaded_status == JobStatus.RUNNING:
                            ...
                            logger.warning(f"Job {job_id} was running during shutdown, marked as FAILED.")
                        else:
                        ...
                        if job_data.get("created_at"):
                            job.created_at = datetime.fromisoformat(job_data.get("created_at"))
                        if job_data.get("started_at"):
                            job.started_at = datetime.fromisoformat(job_data.get("started_at"))
                        if job_data.get("completed_at"):
                            job.completed_at = datetime.fromisoformat(job_data.get("completed_at"))
                        ...
                        logger.warning(f"Invalid job data in {job_file}: missing job_id or job_type")
    ...
    def create_job(
        ...
        logger.info(f"Creating new job: id={job_id}, type={job_type}, created_by={created_by}")
        ...
        with self.lock:
            self.jobs[job_id] = job
            logger.debug(f"Added job {job_id} to jobs dictionary (total jobs: {len(self.jobs)})")
    ...
    def get_jobs(
        ...
        Returns:
            List of jobs
        """
        logger.debug(f"Getting jobs with filters: status={status}, job_type={job_type}, created_by={created_by}")
    ...
    def run_job_async(
        ...
        def _run_job():
            ...
            try:
                ...
                logger.debug(f"Task function for job {job.job_id} completed successfully")
                job.complete(result)
    ...
    def cancel_job(self, job_id: str) -> bool:
        ...
        if job.status in [JobStatus.QUEUED, JobStatus.RUNNING]:
            ...
            logger.info(f"Cannot cancel job {job_id}: job is already in {job.status} state")
            return False
                ...
                if job.status in [JobStatus.COMPLETED, JobStatus.FAILED, JobStatus.CANCELLED]:
                    if job.completed_at:
                        ...
                        if age > max_age_hours:
                            logger.debug(f"Marking job {job_id} for removal (age: {age:.1f} hours)")
