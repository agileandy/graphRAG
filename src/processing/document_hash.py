...
def enrich_metadata_with_hashes(
    metadata: dict[str, Any], document_text: str | None = None
) -> dict[str, Any]:
...
def is_likely_duplicate(
    ...
    if "content_hash" in enriched_metadata:
        try:
            results = collection.get(
                where={"content_hash": enriched_metadata["content_hash"]}, limit=1
            )

            if results and results["ids"] and len(results["ids"]) > 0:
                logger.info(
                    f"Duplicate detected by content hash: {enriched_metadata.get('title', 'Untitled')}"
                )
    ...
    if "metadata_hash" in enriched_metadata:
        try:
            results = collection.get(
                where={"metadata_hash": enriched_metadata["metadata_hash"]}, limit=1
            )

            if results and results["ids"] and len(results["ids"]) > 0:
                logger.info(
                    f"Duplicate detected by metadata hash: {enriched_metadata.get('title', 'Untitled')}"
                )
