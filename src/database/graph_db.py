...
class GraphDatabase:
    ...
    def add_document(self, document_id: str, metadata: Dict[str, Any]) -> Dict[str, Any]:
        """
        ...
        for field in ["author", "source", "url", "publication_date", "category", "tags"]:
        ...
        result = self.neo4j_db.run_query_and_return_single(query, {"properties": properties})
        
        return result
    
    def add_concept(self, concept_name: str, properties: Optional[Dict[str, Any]] = None) -> Dict[str, Any]:
        """
        ...
        existing = self.neo4j_db.run_query_and_return_single(query, {"name": concept_name})
        
        if existing:
            logger.debug(f"Concept '{concept_name}' already exists with ID {existing['id']}")
            return existing
        ...
        result = self.neo4j_db.run_query_and_return_single(query, {"properties": concept_properties})
        
        return result
    
    def add_document_concept_relationship(self, 
                                         document_id: str, 
                                         concept_id: str,
    ...
    def get_documents_by_concept(self, concept_id: str, limit: int = 10) -> List[Dict[str, Any]]:
        """
