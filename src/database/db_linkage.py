...
class DatabaseLinkage:
    ...
    def hybrid_search(
        ...
        if not is_healthy:
            ...
            success, repair_message = self.vector_db.repair_index()
            if not success:
                # If repair failed, return an error
                return {
        ...
        try:
            ...
            error_message = str(e)
            print(f"Error during vector search: {error_message}")

            # Return empty vector results but continue with graph search
            vector_results = {
                "ids": [[]],
                "documents": [[]],
                "metadatas": [[]],
