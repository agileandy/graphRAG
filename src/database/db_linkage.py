...
class DatabaseLinkage:
    ...
    def hybrid_search(self,
        ...
        graph_results = []
        seen_concept_ids = set()  # Track seen concept IDs to avoid duplicates
        ...
        for i, chunk_id in enumerate(vector_results.get("ids", [[]])[0]):
            ...
            if concept_id:
                try:
                    ...
                    for record in records:
                        # Only add if we haven't seen this concept ID before
                        if record["id"] not in seen_concept_ids:
                            ...
                            seen_concept_ids.add(record["id"])
                except Exception as e:
