#!/bin/bash
# Service script for the Bug Tracking API server
...
start_server() {
    echo "Starting Bug Tracking API server on $HOST:$PORT..."
    
    # Check if the server is already running
    if pgrep -f "python.*bugAPI.py" > /dev/null; then
        echo "Server is already running."
        return
    fi
    
    # Start the server in the background
    nohup python "$PROJECT_ROOT/bugMCP/bugAPI.py" \
        --host "$HOST" \
        --port "$PORT" \
        --log-level "$LOG_LEVEL" \
        > "$PROJECT_ROOT/bugMCP/bugapi.log" 2>&1 &
...
stop_server() {
    echo "Stopping Bug Tracking API server..."
    
    # Find and kill the server process
    pkill -f "python.*bugAPI.py"
...
check_status() {
    if pgrep -f "python.*bugAPI.py" > /dev/null; then
        echo "Bug Tracking API server is running."
        
        # Get the process ID
        PID=$(pgrep -f "python.*bugAPI.py")
        echo "Process ID: $PID"
        ...
        echo "Bug Tracking API server is not running."
    fi
}
